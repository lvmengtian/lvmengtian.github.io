...
### **面试回答模板**
> “面试官您好，关于Spring循环依赖：
> 1.  **大部分通过属性或Setter注入的单例Bean，Spring是能够解决的**。
> 2.  它的解决方案是著名的**三级缓存**。一级缓存存放成品，二级缓存存放半成品，而**三级缓存存放的是一个ObjectFactory，这是解决循环依赖和AOP代理协同工作的关键**。
> 3.  核心思想是**提前暴露对象实例**，打破‘鸡生蛋蛋生鸡’的循环。
> 4.  **构造器注入** 和 **Prototype作用域** 的循环依赖是无法解决的，会抛出 `BeanCurrentlyInCreationException`。”
