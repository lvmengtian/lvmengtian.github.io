...
## 什么是MVCC， 可以解决什么问题
> 想象一下，假期里你和朋友同时在看同一篇爆款笔记。你在读的时候，他‘手滑’把笔记删了。请问：你应该看到他删除后的结果吗？还是应该继续安心读完？
> 数据库每天都在处理海量这样的‘并发访问’冲突。而 **MVCC (Multi-Version Concurrency Control) **，就是让数据库既能保证你读数据时不被阻塞，又能保证你朋友删数据时不报错的神奇机制！

### 🔍 白话核心概念

**MVCC是什么？**
**多版本并发控制**。通俗讲，就是数据库**为了同一行数据保留了多个历史版本**。当不同的事务来读写这行数据时，数据库会根据规则给它分配合适的版本，让它们像在‘不同的时空’里操作一样，互不干扰。

**它解决了什么核心问题？**
1.  **解决`脏读`、`不可重复读`问题**：通过版本控制，一个事务只能读到在它开始之前就已经提交的数据版本。
2.  **提高并发性能**：**读不阻塞写，写不阻塞读**。这是MVCC带来的最大性能优势，避免了大量的锁等待。

### 🎤 图解MVCC
**MVCC的两大核心“道具”：**
1.  **隐式字段**：
    -   `DB_TRX_ID` (6字节)：记录最近一次**更新**这行数据的事务ID。
    -   `DB_ROLL_PTR` (7字节)：**回滚指针**，指向这条记录的上一个历史版本（存储在Undo Log里）。
    -   `DB_ROW_ID` (6字节)：隐含的自增ID（如果没有主键）。
    -   还有一个删除标记位。

2.  **Read View（读视图）**：
    -   这是事务进行**快照读**（普通SELECT）时产生的“一致性视图”。
    -   它决定了当前事务能看到哪个版本的数据。
    -   它包含几个关键ID：
        -   `m_ids`: 生成Read View时，系统中**活跃**（未提交）的事务ID集合。
...
我们通过下面的流程图，来展示MVCC如何通过`Read View`和`Undo Log`来决定一个事务应该看到哪个版本的数据：
![](/images/mvcc_flow.png)

### 🌰 场景示例

**场景：** 事务A和事务B并发执行。
1.  事务A (ID=50) 开始，查询一条数据`balance=100`。
2.  事务B (ID=60) 开始，将`balance`更新为`200`并**提交**。
3.  事务A再次查询`balance`。

**问：事务A第二次查询到的值是多少？**

**答：**
- 如果事务隔离级别是 **`REPEATABLE READ` (可重复读)**：事务A**仍然看到100**。因为它在第一次查询时就生成了一个Read View，之后一直使用这个视图，所以看不到事务B提交的修改。**这就解决了‘不可重复读’。**
- 如果事务隔离级别是 **`READ COMMITTED` (读已提交)**：事务A**会看到200**。因为它在每次查询时都会生成一个新的Read View，所以能看到最新已提交的数据。
